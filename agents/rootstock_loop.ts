...
/**
 * ====== CONFIG (Rootstock-native) ======
 * Set these env vars for Rootstock testnet/mainnet:
 *   export RPC_URL=https://public-node.testnet.rsk.co
 *   export CHAIN_ID=31
 *   export POOL_ADDR=<Aave/Pool address on Rootstock>
 *   export WETH_ADDR=<WETH or RBTC wrapper address>
 *   export USDC_ADDR=<USDC or stablecoin address>
 *   export ROUTER_ADDR=<Uniswap/Sovryn/AMM router address>
 *   # Optionally: export FEE_TIER, LOOPS, BORROW_USDC_PER_LOOP, INITIAL_SUPPLY_WETH
 *
 * Defaults are for Ethereum mainnet. Override for Rootstock!
 */
const CHAIN_ID = Number(process.env.CHAIN_ID || 31); // 31 = Rootstock testnet
const FEE_TIER = Number(process.env.FEE_TIER || 3000);
const LOOPS = Number(process.env.LOOPS || 1);
const BORROW_USDC_PER_LOOP = BigInt(process.env.BORROW_USDC_PER_LOOP || 25_000_000n);
const INITIAL_SUPPLY_WETH = BigInt(process.env.INITIAL_SUPPLY_WETH || 100_000_000_000_000_000n);

// Addresses (override with Rootstock contracts)
const POOL = getAddress(process.env.POOL_ADDR || "0x0000000000000000000000000000000000000000"); // Set to deployed Pool
const WETH = getAddress(process.env.WETH_ADDR || "0x0000000000000000000000000000000000000000"); // Set to RBTC wrapper or WETH
const USDC = getAddress(process.env.USDC_ADDR || "0x0000000000000000000000000000000000000000"); // Set to stablecoin
const ROUTER = getAddress(process.env.ROUTER_ADDR || "0x0000000000000000000000000000000000000000"); // Set to AMM router
